...
%\VignetteDepends{tuneR,sound,audio}
...
\begin{document}
\maketitle
This document shortly details how to import and export sound with \texttt{R} using the packages \seewave, \sound, \tuneR~and \audio.
...
\section{In}
The main functions of \texttt{seewave (>1.5.0)} can use different classes of objects to analyse sound: (\emph{i}) usual classes (numeric \texttt{vector}, numeric \texttt{matrix}), (\emph{ii}) time series classes (\texttt{ts}, \texttt{mts}), and (\emph{iii}) sound-specific classes (\texttt{Wave}, \texttt{Sample}, \texttt{audioSample}). 
...
There are three object classes corresponding to the binary \texttt{wav} format: (\emph{i}) the class \texttt{Wave} of the package \tuneR, (\emph{ii}) the class \texttt{Sample} of the package \sound, and \emph{iii} the class \texttt{audioSample} of the package \audio. There is currently no other way to import sound files. In particular, there is no function to read \texttt{.mp3} or \texttt{.aiff} files.
...
\subsubsection{Sample class (package sound)}
...
s8$duration
@
Unlike \texttt{readWave}, \texttt{loadSample} changes the limits of the sound between $\pm 1$:
<<rangeSample>>=
range(s8$sound[1,])
@ 

\seewave~functions which return a value describing a sound have a special argument named \texttt{output} that can be set to \texttt{"matrix"}, \texttt{"Sample"}, \texttt{"audioSample"}, \texttt{"Wave"} or \texttt{"ts"}. This allows to control the class of the returned object.
...
However, matrix objects are easier to handle and can be used with functions not dedicated to sound. It might be then better to let the arguments \texttt{Sample} and \texttt{Wave} set to \texttt{FALSE} and to store the sampling frequency \texttt{f} in an object:
<<>>=
f<-8000
oscillo(s2,f)
@
...
\tuneR, \sound and \audio have each funcion to write \texttt{.wav} files: \texttt{writeWave}, \texttt{saveSample}, and \texttt{save.wave} respectively.  Within \seewave The function \texttt{savewav}, which is based on \texttt{saveSample}, can be used to save data as \texttt{.wav}. By default, the name of the object will be used for the name of the \texttt{.wav} file:
<<savewave>>=
...
Both \texttt{Sample} and \texttt{Wave} classes can handle stereo files. There are some specific functions regarding mono\slash stereo type. Both libraries include functions with the same name.
...
\textbf{seewave} & -- & export, savewav & -- & listen & \texttt{vector}, \texttt{matrix}, \\ 
 ...
 & & & & &  \texttt{Sample}, \texttt{audioSample}\\ 
\hline 
\end{tabular} 
\end{document}
